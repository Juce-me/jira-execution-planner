...
| --- | --- | --- | --- |
| ENG Board progressive and batched Epic discovery, Story progress, and retained partial results | `backend/routes/eng_board_routes.py`, `backend/services/eng_board.py`, `frontend/src/eng/useEngBoardData.js`, `frontend/src/eng/EngBoardEpicCard.jsx` | Passive loading, bounded Component-index requests, and recovery presentation add no product action. Keep the existing terminal `api_result` event; emit no events for Component batches, candidate indexes, pages, skeleton animation, or provisional counts. No trigger, event type/name, feature/page name, typed parameters, GA4 dimensions, or runbook steps change. Existing Board analytics regression coverage continues to guard terminal-only reporting. | 2026-09-10 |
