...
- [ENG shared Sprint selector availability](EXEC-eng-board-scope-selector-availability.md): implemented and verified on `improvement/eng-board-scope-selector-availability`; awaiting merge and acceptance, so it remains `EXEC-*`. The shared semantic selector separates scope intent from load authority, exposes explicit loading/setup/error states, preserves ordinary Sprint access across all five ENG modes, and makes explicit Project Track Sprint commitments perform one current source-only reload even with prewarmed range/per-Sprint caches. A current-Sprint commitment repairs a divergent range once; an already matching reselect remains a no-op. Its 22-cell two-scope authority/readiness matrix and config/stream auth-lock interaction negatives pass within the serial 161-test browser campaign, alongside frontend unit, build, backend safety, and full Python checks. Supersedes only the prior no-op selection requirement; Sprint catalog implementation remains separately owned. No existing authenticated local server was available, so positive authenticated cross-sprint loading remains unverified.
- [Persistent Sprint and per-Sprint Team catalogs](EXEC-persistent-sprint-team-catalogs.md): fresh review 2026-09-13 requires four amendments before execution: browser Sprint authority/empty handling, async refresh completion, budget-aware OAuth transport, and first-config-write fencing. Workspace-shared ownership remains confirmed. Recommended after the selector availability plan, with sequential dashboard integration and the selector matrix rerun. No implementation or publication performed.
...
11. `EXEC-shared-header-dropdown-width-contract.md`
   - Implementation and code verification are locally complete on `bugfix/shared-dropdown-width-contract` and merged with the current onboarding implementation from `origin/main`. The build, 1,144 frontend units, 1,538 isolated Python tests with 9 skips, all 72 dropdown-suite tests, 100 onboarding-tour tests, and 220 onboarding integration tests pass; this plan remains `EXEC-*` pending acceptance. Configured DB-mode startup verification is unavailable while local PostgreSQL is down, and the original ignored before-state screenshots are no longer present.
