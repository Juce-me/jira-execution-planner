...
The database can store Home project metadata cache rows keyed by workspace, root goal, sub-goal, lifecycle tab, and cache version. This should be done only after measuring that database caching reduces Jira/Home fan-out without making first load stale or harder to reason about.
