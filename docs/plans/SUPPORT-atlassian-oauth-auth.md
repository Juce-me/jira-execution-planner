...
5. Start the server and open `/login`. If no OAuth session exists, the app should show a `Sign in with Atlassian` action. That action starts Atlassian OAuth; for managed Atlassian accounts backed by Microsoft Entra SSO, the flow should redirect through Microsoft automatically, then show the Atlassian authorization/consent screen for this app before returning to the app callback. Confirm `/api/auth/status` reports `authenticated: true`, then use `/api/test` to verify Jira REST access. Current DB/OAuth dashboard and EPM route behavior is documented in `INSTALL.md`, `docs/SUPPORT-atlassian-oauth-setup.md`, and `docs/plans/DONE-04-db-user-home-epm-read-token.md`.
