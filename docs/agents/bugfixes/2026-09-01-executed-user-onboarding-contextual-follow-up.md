...
Accurate as executed on `feature/user-onboarding-tour` through post-review build commit `d27d449`. The shipped behavior follows the approved contextual-module, explicit-Next, single-favorite-control, accessibility-restoration, analytics, and no-mutation contracts. The final review also proved that rejected terminal persistence remains retryable, contextual destination focus reaches programmatically focusable regions without adding tab stops, constrained coachmark placement cannot bypass an enabled launcher, and an editable unconfigured workspace routes only an active contextual Configuration launch to Departments → Team Groups. Ordinary Settings routing remains unchanged.

## Outcome

The desktop follow-up and its final whole-branch review fixes are implemented and verified. Catch Up hands off to Configuration, Planning, Board, and Statistics only through native user activation of the real launchers; interactive Jira-field previews keep Next locked until an exact settled preview closes and never auto-advance; failed final persistence preserves the retry path; constrained placement never turns an available launcher into manual completion; and destination focus/description handoff now lands on the real Planning, Board, and Statistics regions. The coachmark action hierarchy is stable at normal and narrow desktop widths, and Department Settings retains one left-row favorite plus a compact right-pane visibility row. Final verification passed 1,396 Python tests with 7 skips, 1,044 frontend unit tests, and 588 Playwright tests with 2 skips under the pinned runtimes. Generated output reproduced cleanly from `d27d449`, and all 11 required settled screenshots passed original-resolution inspection. Remote integration remains intentionally pending explicit user direction.
