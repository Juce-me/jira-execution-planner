...
| --- | --- | --- | --- |
| ENG Board progressive Epic discovery, Story progress, and retained partial results | `backend/routes/eng_board_routes.py`, `frontend/src/eng/useEngBoardData.js`, `frontend/src/eng/EngBoardEpicCard.jsx` | Passive loading and recovery presentation add no product action. Keep the existing terminal `api_result` event; emit no events for candidate indexes, pages, skeleton animation, or provisional counts. No trigger, event type/name, feature/page name, typed parameters, GA4 dimensions, or runbook steps change. Existing Board analytics regression coverage continues to guard terminal-only reporting. | 2026-09-10 |
...
| ENG inline issue-field query typing, cancel/dismiss, and derived recalculation | `frontend/src/issues/IssuePersonEditor.jsx`, `frontend/src/issues/StoryPointsEditor.jsx`, `frontend/src/eng/useEngIssueFieldEdits.js`, `frontend/src/dashboard.jsx` | Search keystrokes and cancel/dismiss are incidental editor state, while Epic totals and Planning recalculation are derived state after a tracked result. Emit no per-keystroke, cancel, dismissal, or recalculation event; the bounded `open`, `submit`, and `result` workflow is authoritative and never includes query text, people identity, issue keys, field IDs, Story Point values, or raw errors. | 2026-09-09 |
...
| Header dropdown query typing, width stabilization, and Sprint loading/error visibility | `frontend/src/dashboard.jsx`, `frontend/src/styles/shared/header.css`, `frontend/src/styles/shared/controls.css` | Query text only filters already-loaded local options and is discarded on close; stable sizing and opening the Sprint menu to read or retry its passive loading/error state have no independent product outcome. The retry repeats `GET /api/sprints`, while existing `filter_changed` events remain attached to committed Sprint, Group, and Teams selections. No separate `userevent` is emitted, and raw query text, sprint names, group names, and team names are never collected. | 2026-09-10 |
