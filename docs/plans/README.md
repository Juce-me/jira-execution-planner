...
- [Persistent Sprint and per-Sprint Team catalogs runtime repair](EXEC-persistent-sprint-team-catalogs-runtime-repair.md): source-contract repair and focused red/green verification complete for the legacy cache-busters that caused strict catalog requests to return `400`. The production bundle was rebuilt and local PostgreSQL/OAuth startup passed. Served-bundle, authenticated browser, two-user/cross-workspace, and cached-latency acceptance remain open before the feature can be reported as working.
