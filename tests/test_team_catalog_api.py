...
class TestTeamCatalogAPI(unittest.TestCase):
    def setUp(self):
        ...
        self._groups_path = os.path.join(self._tmpdir, 'team-groups.json')
        ...
        self._groups_patcher = patch.object(jira_server, 'resolve_groups_config_path', return_value=self._groups_path)
    ...
    def tearDown(self):
        ...
        for f in [self._catalog_path, self._dashboard_path, self._groups_path]:
            if os.path.exists(f):
                os.unlink(f)
