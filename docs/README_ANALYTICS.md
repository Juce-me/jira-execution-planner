...
| `settings_action` | `section`, `workflow_action`, `result`, `validation_count_bucket`, `dirty_state`; Department visibility and first-run selection use bucketed `selected_count_bucket`/`group_count_bucket` only | Settings tab open, test, save, cancel, validation failure, app-admin membership save, Department visibility preference change, first-run Department selection completion, or shared excluded-capacity epic toggle from Planning/Reporting. Tab-open also fires from the ENG Board's first-run *Configure Group Board* link (`section=boards`, `source_surface=board`) | `frontend/src/settings/*`, `frontend/src/dashboard.jsx` | browser | Settings reliability |
