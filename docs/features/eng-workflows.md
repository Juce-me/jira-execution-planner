...
The shared Sprint selector remains clickable while sprint discovery is pending or returns no options. It shows **Loading…** while closed, and its open menu shows **Loading sprints...** or **No sprints available**, so the control never becomes an inert loading indicator.
