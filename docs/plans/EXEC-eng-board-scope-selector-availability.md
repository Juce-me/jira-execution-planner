...
> **Status: Planned; not implemented.** Investigated 2026-09-13 on `bugfix/board-progressive-loading`, HEAD `40bffe730159ebadb1904b0188981c853fdf85ed`. Live no-op behavior and isolated runtime reproductions agree on the selection/load-authority conflation. The exact live bootstrap capability value and underlying storage response remain unobserved; do not present a storage migration or capability defect as proven.
>
> **Execution:** Use `superpowers:executing-plans` task by task after implementation is requested. This document authorizes no execution, commit, push, publication, worktree, shared configuration change, or Jira write. Preserve the substantial existing issue #190 diff. This plan supersedes only the disabled/no-op scope-selection requirements in [the scope follow-up](EXEC-eng-board-scope-performance-regressions-190.md); its backend authority and measurement protections remain required. [Progressive loading](EXEC-eng-board-progressive-loading-190.md) remains authoritative for the stream, bounds, incomplete results, and measurement.
...
### P2 — existing tests and documentation encode the rejected behavior

- `tests/ui/eng_group_board_view.spec.js:352–374` explicitly expects disabled options and dispatched clicks to do nothing after a valid catalog. Eligibility cases at `:377–405` similarly conflate unavailable loading with unavailable selection. The delayed-config case at `:631` validates eventual enablement but not accepted selection during the wait.
- Five focused existing tests passed during investigation, including both missing-authority no-op tests. Their green result therefore does not satisfy this request. Preserve their negative network assertions while replacing their UI expectations.
- `docs/features/eng-workflows.md:67` describes capability/authority as selection prerequisites; `:71` also contradicts its earlier pending-catalog paragraph. `docs/README_ANALYTICS.md:128` describes ignored clicks. Update the Board selection wording during implementation without changing the separately owned Sprint-catalog contract.
...
- Visually inspected the live open menu and the settled synthetic missing-authority screenshot. No after-fix image or live request-count/performance claim exists because no fix was implemented. The browser was returned to its original ordinary Sprint and Catch Up mode after validation.
...
Existing files were verified during investigation. Recheck every path before editing; `Create` entries are deliberate new files. Do not broaden to catalogs or backend implementation.
...
| Modify | `frontend/src/dashboard.jsx` | Integrate model, accepted bootstrap status/generations, shared semantic dropdown, explicit Board state, requested-versus-loading guards, refresh and revision wiring |
...
| Modify after implementation | `docs/features/eng-workflows.md`, `docs/README_ANALYTICS.md`, `docs/ontology.md` | Correct selection-vs-load wording and analytics allowlist; retain catalog/stream facts |
| Modify after implementation | `docs/postmortem/MRT027-board-catalog-pagination.md` | Amend the prevention bullet requiring disabled missing-authority options; preserve historical evidence. Read its local instructions again |
| Modify after implementation | this plan, `docs/plans/README.md`, `docs/plans/EXEC-eng-board-scope-performance-regressions-190.md` | Accurate outcome, supersession note for obsolete no-op requirement; preserve historical tests |
...
Read-only contracts: `frontend/src/eng/useEngSprintData.js`, `frontend/src/eng/useStrictEngBoardIntegration.js`, `frontend/src/eng/useEngBoardData.js`, `frontend/src/eng/EngBoardView.jsx`, `frontend/src/eng/EngModeControl.jsx`, `frontend/src/eng/engModeState.js`, `frontend/src/api/engBoardApi.js`, `frontend/src/settings/groupConfigUtils.js`, `frontend/src/ui/EmptyState.jsx`, `frontend/src/ui/LoadingState.jsx`, `backend/security/CONFIGURATION_OWNERSHIP.md`, `backend/routes/settings_routes.py`, `backend/routes/eng_board_routes.py`, `backend/services/eng_board.py`, `backend/security/policy.py`, and existing Board backend tests. Reuse their current APIs; readiness presentation can be a conditional sibling of `EngBoardView` in dashboard, so no Board renderer redesign is required.

Current planning-only edits: create this file; add its own index link and a clearly labelled pending-selector ontology entry. Other dirty files remain untouched. Another task added a catalog plan/index entry during investigation; preserve it. The two efforts overlap in dashboard integration during future execution, so reconcile current source before editing, without reverting either task's changes.
...
- [ ] Red command: `npx --no-install playwright test tests/ui/eng_group_board_view.spec.js --workers=1 -g 'selector regression:'`. Expected current failure: no semantic option/accepted selection/configuration state. Do not count timeout caused by missing fixture onboarding as a valid red.
- [ ] Keep these tests red until Tasks 2–4. Preserve all unrelated issue #190 tests and endpoint-shaped authority fixtures. Do not import a `.spec.js` from another spec.

### Task 2 — model selection separately from readiness

**Files:** new model and unit test, dashboard integration.

- [ ] Add table tests for pending/failed bootstrap, capability false/null/malformed, missing columns, missing saved authority, Component-only, Team-only, no group, group pending/failure, valid projects and saved Board fallback. All Board options are selectable with a usable catalog; only eligible scope loads. Ordinary selection eligibility never changes across Board states.
- [ ] Red: `node --test tests/test_eng_sprint_selector_state.js`; expected missing module first, then expected-state mismatches while integrating.
- [ ] Implement this pure decision contract (input names below are the new helper interface, not renamed persistence fields):
...
- Preserve all existing dirty issue #190 backend, owner, tests, measurement and generated changes. The current diff adds saved authority gating and catalog readiness as well as progressive loading; this plan changes only the **selection semantics and wiring** around those facts. Do not reset, restore, commit, push or rewrite history. Do not publish an execution handoff referencing unpublished artifacts.
...
## 10. Plan review and residual risks
...
Residual risks: live capability/raw configuration response provenance remains unknown; actual saved authority may require an operator configuration decision outside this fix. Concurrent catalog work may move the cited dashboard lines. The current owner caches by revision; regression tests must prove changed Department authority invalidates it before execution is accepted. Mode-specific task/compute flows differ, so assertions must count actual lane/source requests rather than impose an invented single-request model. No positive authenticated cross-sprint success or performance improvement is claimed by this planning task.
