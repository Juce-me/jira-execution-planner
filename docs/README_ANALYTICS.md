...
- Operational load-performance history is allowlisted outside GA4: debug observations are stored in `load_performance` and read only by workspace tool admins. Accepted Component observations persist and remain filterable as `scopeType=component` with `sprintId=null`; All work likewise uses no fabricated Sprint. Group/sprint identifiers stay in the application database, never the dataLayer. Collection, history filtering, expansion and telemetry HTTP calls emit no product analytics events: their purpose is internal latency diagnosis, and self-instrumentation would distort measurements. The strict ENG Board emits one existing `api_result` only when its current generation reaches a terminal result; superseded or auth-locked generations emit none.
...
| `external_link_opened` | `link_type`, `source_surface`, `result`; plus `issue_count_bucket`/`issue_kind` for `jira_issue_list`, `issue_kind` for `jira_issue_browse`, or `epm_tab`/`project_scope` for `jira_home_project|jira_home_update`. Strict Board Work items use `issue_kind=mixed`; Epic and legacy Story actions remain `epic` and `story`. Story-requirement links use `link_type=jira_issue_browse`, `issue_kind=epic`, and `source_surface=catch_up|planning`. | Jira issue-list/filter, direct Jira issue/epic/initiative browse, Story-requirement Epic browse, or Jira Home project/update link opens | `frontend/src/components/TrackedExternalLink.jsx`, `frontend/src/components/JiraExportButton.jsx`, `frontend/src/jiraExportUtils.mjs`, `frontend/src/dashboard.jsx`, `frontend/src/stats/*`, `frontend/src/eng/StoryRequirementCard.jsx`, `frontend/src/epm/*` | browser | External Jira/Home link usage |
| `settings_action` | `section`, `workflow_action`, `result`, `validation_count_bucket`, `dirty_state`; Department preference changes may use bucketed `selection_count_bucket`, first-run selection uses `group_count_bucket` only, and workspace revision conflicts use `conflict_state=remote` plus bucketed `conflict_count_bucket`. Onboarding uses exactly `section=onboarding`, `workflow_action=started|completed|skipped`, `source_surface=first_run|settings`, and `module_id=catch-up|configuration|planning|board|statistics`; `completed` and `skipped` add `result=success` only after that module's preference write succeeds, while `started` has no `result`. | Settings tab open, test, save, cancel, validation failure, workspace revision conflict (`workflow_action=save_result`, `result=failure`), app-admin membership save, Department visibility preference change, first-run Department selection completion, onboarding start/completion/skip, or shared excluded-capacity epic toggle from Planning/Reporting. Tab-open also fires from the ENG Board's first-run *Configure Group Board* link (`section=boards`, `source_surface=board`). Step navigation is intentionally untracked: Next, Back, target activation, target visibility, and displayed step content create no new onboarding event because they are transient and would add noise or risk collecting issue, Department, Team, sprint, search, and other raw context. Field-preview activation retains the normal safe `priority_options_open`, `project_track_options_open`, or `status_options_open` event owned by the underlying control; it does not add a parallel tour event. | `frontend/src/settings/*`, `frontend/src/onboarding/useOnboardingTour.js`, `frontend/src/dashboard.jsx` | browser | Settings and onboarding reliability |
...
| `planning_action` | `feature_name=planning_capacity_edit`, `workflow_action` (`capacity_edit_open`\|`capacity_change_submit`\|`capacity_change_result`), `source_surface=planning`; `result` (`success`\|`failure`\|`conflict`) only for `capacity_change_result` | Opening a configured Planning capacity editor, submitting its change, or receiving its final result | `frontend/src/eng/PlanningTeamCapacityCards.jsx`, `frontend/src/analytics/dashboardAnalytics.js` | browser | Capacity-edit adoption and reliability |
...
### Planning Capacity Editing

Capacity editing uses the existing browser `userevent` transport with `trigger=userevent`, `event_type=event`, and canonical `event_name=planning_action`. Its fixed identity is `feature_name=planning_capacity_edit`, `source_surface=planning`, and the three allowlisted `workflow_action` values above. The result field is omitted for editor-open and submit events; it is emitted only for `capacity_change_result`.

The browser-observed capacity API reliability event uses the existing `api_result` contract with `api_surface=jira_team_capacity`. Neither event may carry a capacity value, issue key, team or sprint name, Jira URL, custom-field id, JQL, raw error, account/workspace/cloud identifier, user text, token, or config payload. The Jira icon remains covered by the existing `external_link_opened` event with `link_type=jira_issue_browse` and `source_surface=planning`.

This uses the existing GTM `userevent` trigger and existing GA4 property keys. Do not register GA4 custom dimensions for capacity editing.

### ENG Inline Issue Field Editing

Inline Assignee, Delivery Owner, and Story Points editing uses canonical `issue_field_edit_action` on the existing `userevent` transport. The fixed builder accepts only the workflow, field, issue-kind, surface, and result enums listed above. Open and submit omit `result`; the final result event requires it. The related browser reliability event uses `api_result` with `api_surface=jira_issue_field_edits` and `feature_name=eng_issue_field_edits`.

Neither event may carry search queries, display names, emails, account IDs, issue keys, Jira field IDs, Story Point values, raw errors, URLs, JQL, or response bodies. `field_name` is mapped through the shared GTM tag but remains intentionally unregistered as a custom dimension until a named report requires it; do not bulk-register allowlisted parameters.
...
| --- | --- | --- | --- |
| Persistent Sprint/Team catalog read, fill, refresh, completion, and disabled availability | `backend/services/workspace_catalog_cache.py`, `backend/services/catalog_refresh_runtime.py`, `backend/routes/settings_routes.py`, `backend/routes/eng_routes.py`, `frontend/src/dashboardRuntime.js`, `frontend/src/settings/teamAvailability.js`, `frontend/src/settings/useTeamCatalogLifecycle.js`, `frontend/src/dashboard.jsx` | Passive catalog transport and authority state only. Analytics contract: trigger none; `event_type=none`; `event_name=none`; `feature_name=eng/settings`; typed params none. Cache restoration, validated empty replacement, retained matching failures, pending/completed attempts, Retry, stale-response suppression, and unavailable Team controls add no `dataLayer` event. Existing explicit Sprint/Team selection events remain unchanged. Never send catalog identity/version, attempt id, browser context, scope digest, Board/Sprint/Team ids or names, failure codes, deadlines, or raw errors. The two app-owned transports remain only `pageview` and `userevent`; no GA4 runbook change is required. | 2026-09-14 |
| ENG Board Sprint selection/readiness separation, cross-sprint availability, progressive discovery, loading animation, and retained partial results | `frontend/src/dashboard.jsx`, `backend/routes/settings_routes.py`, `backend/routes/eng_board_routes.py`, `backend/services/eng_board.py`, `frontend/src/eng/useEngBoardData.js`, `frontend/src/eng/loadPerformance.js`, `frontend/src/eng/EngBoardView.jsx` | Sprint cache hits/misses, pending/empty catalog gates, capability, saved Jira project/Board authority, Department Board columns, readiness transitions, retries, stale-response rejection, candidate indexes, Component pages, Team-parent batches, provisional counts, loading bars, paint observations, Project Track source-range synchronization/cache bypass, and incomplete/stale presentation are passive state and emit no new event. A current-Sprint Project Track commitment that repairs a divergent range performs its one source reload but emits no `filter_changed`; the same choice with an already matching range emits no event or request. Accepting Component or All work into a loading/setup/error state emits the existing `trigger=userevent`, `event_type=event`, canonical `event_name=filter_changed`, `feature_name=dashboard`, `filter_type=sprint`, `source_surface=board`, bounded `scope_type=component|all_work`, and matching bounded `sprint_selection_state`, exactly once. Search typing, a same-state reselect, automatic readiness, configuration retry completion, post-lock interaction, and delayed completion after a config or stream `401` emit none. Tests distinguish accepted pre-lock choices from the empty post-lock event sink and attempt the hidden selector by pointer plus keyboard focus traversal after lock. All values are safe strings/enums; never send Sprint, Department, group, project, or issue ids/names, query text, readiness reasons, URLs, JQL, frame payloads, or raw errors. The terminal `api_result` contract is unchanged: `feature_name=eng_board`, `api_surface=eng_board`, bounded `scope_type`, existing method/status/result/duration/cache parameters, and one terminal numeric duration, at most once for an accepted current generation and never for superseded or auth-locked loads. Operational observations remain outside GA4. No taxonomy, custom-definition, transport, consent, or runbook change is required. | 2026-09-14 |
| Cache-first Sprint startup restoration | `frontend/src/dashboard.jsx`, `backend/services/sprints.py` | Persisting and restoring the saved Sprint id and label, validating them through the established cached catalog request, and temporarily disabling cross-sprint choices are passive startup behavior only. Existing `filter_changed` remains limited to user-committed Sprint choices; startup restoration emits no event and never sends Sprint ids or names. | 2026-09-10 |
| ENG filter-driven alert filtering | `frontend/src/eng/engAlertFilters.js`, `frontend/src/eng/useEngAlertFilters.js`, `frontend/src/dashboard.jsx`, `frontend/src/eng/EngAlertsPanel.jsx` | Filtering alert rows, category chips, and the total count is derived presentation for the existing debounced `app_search` and `filter_changed` actions. Add no parallel alert event; existing events remain bucketed and must never contain query text, issue keys, summaries, Team/Initiative names, Jira URLs, or JQL. | 2026-09-17 |
| ENG Stories Required alert-to-ghost navigation | `frontend/src/eng/EngAlertsPanel.jsx`, `frontend/src/eng/alertEpicNavigation.js`, `frontend/src/eng/StoryRequirementCard.jsx` | Local filter clearing, reveal, scroll, focus, and highlight are navigation mechanics rather than a separate product outcome. The hierarchy ghost reuses `external_link_opened` with fixed `link_type=jira_issue_browse`, `issue_kind=epic`, and `source_surface=catch_up|planning`; the alert row adds no separate external action. No event may contain Epic keys, Team or sprint names, labels, Jira URLs, JQL, readiness reasons, or raw counts. | 2026-09-16 |
| ENG epic-header visible-height, fit, and full-value readout correction | `frontend/src/dashboard.jsx`, `frontend/src/styles/eng/issues.css` | Corrective presentation and accessibility only: the title, key, status, Story Points, person, icons, and existing Included/Excluded control keep their current actions and payloads. Truncation discovery, hover/focus readouts, resize handling, and focus styling emit no event and must never send title, status, person, query, issue-key, or Story Point values. Existing issue-edit, status, priority, Project Track, capacity, link, and API-result contracts remain authoritative. | 2026-09-10 |
| Planning issue-card metadata alignment and alphabetical team-capacity ordering | `frontend/src/issues/IssueCard.jsx`, `frontend/src/eng/PlanningTeamCapacityCards.jsx`, `frontend/src/styles/planning/selection.css` | Corrective presentation only: issue metadata keeps the shared ENG card structure and capacity cards keep the displayed Team-name order as selection and capacity values change. Existing `planning_action` covers checkbox changes; no event is added for derived layout/order, and no Team names, issue keys, capacity values, or Story Points are sent. | 2026-09-10 |
| ENG inline people-editor latency, session cache, and in-place search control | `backend/services/jira_issue_field_edits.py`, `backend/auth/context.py`, `backend/auth/db_context.py`, `jira_server.py`, `frontend/src/eng/useEngIssueFieldEdits.js`, `frontend/src/issues/IssuePersonEditor.jsx` | Parallelizing existing Jira metadata reads, reusing the authenticated display identity, and retaining discovered people in auth-scoped browser memory change no product action or payload contract. The existing `issue_field_edit_action` open/submit/result events and `api_result` duration bucket measure the workflow; cache hits and search focus/typing are incidental state and another event would duplicate them. Never expose people identity, search text, issue keys, Jira field IDs, URLs, or raw timings. | 2026-09-10 |
| ENG inline issue-field query typing, current-value/error restoration, compact story/Epic presentation, cancel/dismiss, and derived recalculation | `frontend/src/issues/IssuePersonEditor.jsx`, `frontend/src/issues/StoryPointsEditor.jsx`, `frontend/src/eng/useEngIssueFieldEdits.js`, `frontend/src/styles/eng/status-transitions.css`, `frontend/src/styles/eng/epics.css`, `frontend/src/dashboard.jsx` | Search keystrokes, preserving the current person until typing, restoring it after rejection, fitting full person names in story and Epic metadata, editor styling, and cancel/dismiss are incidental editor state, while Epic totals and Planning recalculation are derived state after a tracked result. Emit no per-keystroke, restoration, presentation, cancel, dismissal, or recalculation event; the bounded `open`, `submit`, and `result` workflow is authoritative and never includes query text, people identity, issue keys, field IDs, Story Point values, or raw errors. | 2026-09-10 |
| ENG missing-Story-Points alert navigation | `frontend/src/eng/EngAlertsPanel.jsx`, `frontend/src/eng/alertStoryNavigation.js`, `frontend/src/dashboard.jsx` | Local reveal, scroll, highlight, and focus are navigation mechanics rather than an independent product outcome. Activating the inline editor already emits the bounded `issue_field_edit_action` open event without issue keys or Story Point values, so no parallel alert-click event is added. | 2026-09-09 |
...
| ENG subtask status menu label and overflow correction | `frontend/src/styles/eng/subtasks.css`, `tests/ui/eng_status_transitions.spec.js` | Corrective presentation only; the existing `status_options_open`, `status_change_submit`, `status_change_result`, and `api_result` events already cover the unchanged status-transition workflow. A new event would duplicate the same interaction and must not expose status names or issue keys. | 2026-09-09 |
...
| Project Track chart segment/legend hover and Jira epic-list links | `frontend/src/stats/ProjectTrackTotalsBar.jsx`, `frontend/src/stats/ProjectTrackSprintChart.jsx`, `frontend/src/stats/ProjectTrackBreakdownChart.jsx`, `frontend/src/stats/ProjectTrackPhaseChart.jsx` | Tab open (`stats_action` view_change) and the filter-bar controls (Capacity side, Mode, both exclusion checkboxes) remain tracked. Hover readouts and terminal-Epic exclusion are derived presentation with no new event. The Epic-mode assignee `No track` segment reuses `external_link_opened` with `link_type=jira_issue_list`, `issue_kind=epic`, `source_surface=stats`, and a bucketed issue count; it never sends assignee names, Jira URLs, JQL, or Epic keys. | 2026-09-10 |
...
| ENG Board/Catch Up filter facet ticks, chip clears, +n more, passive option visuals, and explicit No Project Track state | `frontend/src/eng/EngFilterBar.jsx`, `frontend/src/eng/engFilterOptionVisuals.js`, `frontend/src/eng/engProjectTrack.js`, `frontend/src/eng/engBoardFilters.js`, `frontend/src/eng/engCatchUpFilters.js`, `frontend/src/styles/eng/filter-bar.css`, `frontend/src/eng/EngBoardView.jsx`, `frontend/src/eng/EngView.jsx` | These controls only re-slice already-loaded data; status colors, existing priority/track visuals, and the local both-unchecked No Project Track state add no request or workflow. Keeping them untracked preserves comparable semantics across Board and Catch Up and sends no facet values, group names, issue keys, or Jira data. | 2026-09-04 |
...
| Desktop contextual onboarding module navigation | `frontend/src/onboarding/OnboardingTour.jsx`, `frontend/src/onboarding/useOnboardingTour.js`, `frontend/src/dashboard.jsx` | Launcher/view and option-open events remain existing canonical events. Module navigation itself is allowlisted as no-new-event because it contains no independent product outcome. Raw target/step/Department/Team/issue data is forbidden. | 2026-09-01 |
...
| Multi-tab terminal OAuth recovery coordination | `frontend/src/components/AuthRequiredGate.jsx`, `frontend/src/api/authRecoveryCoordinator.js`, `frontend/src/dashboard.jsx` | No separate `userevent`; the existing privacy-safe `app_error_shown` event remains authoritative for the user-visible auth interruption, while leader/follower coordination is internal reliability state. Never collect recovery attempt IDs, recovery timestamps, capsule contents, browser-session IDs, selected issue keys, or coordination state. | 2026-09-01 |
| Capacity mapping re-verification | `frontend/src/dashboard.jsx` | Treating an existing unverified mapping as dirty only restores the established Settings Save path. The existing `settings_action` save event covers the committed action; a separate event would duplicate it and must not expose Jira project keys, field ids, verification metadata, or workspace revisions. | 2026-09-02 |
| Future-sprint Capacity empty-state refresh | `frontend/src/eng/PlanningTeamCapacityCards.jsx`, `frontend/src/dashboard.jsx` | The empty-state button repeats only the existing scoped `GET /api/capacity`, which is covered by `api_result` with `api_surface=jira_team_capacity`. A separate `userevent` would duplicate API reliability telemetry and must not expose sprint names, team names, issue keys, or Jira URLs. | 2026-09-02 |
| Sprint team cache hydration and edit lock | `backend/routes/settings_routes.py`, `frontend/src/dashboard.jsx`, `frontend/src/settings/TeamGroupsSettings.jsx` | Automatic cache loading and refresh are Settings lifecycle reliability behavior, not a new user action. Existing Department `settings_action` events continue to cover tab, cancel, and save actions; the established manual Refresh control remains uninstrumented. No event may expose team ids, team names, sprint values, workspace ids, or cache contents. | 2026-09-02 |
...
`module_id` is allowlisted and mapped for onboarding payload validation, but remains unregistered because no named report currently needs it.
