...
| P1 | Make packaging self-contained or explicitly release-zip-only | M | 1-2 days | `pyproject.toml`, `scripts/install.sh`, `.github/workflows/release-latest.yml`, static asset loading | There is one reliable install story. Either `pip install .` produces a runnable app with dependencies/assets, or docs clearly say the release zip is the runnable package. |
...
| P2 | Replace obsolete quickstart docs | S | 0.5-1 day | `README.md`, `AGENTS.md` | Onboarding no longer points users toward system/user pip or legacy Basic credentials as the default DB/OAuth path. `QUICKSTART_ENV.txt` was removed in the root cleanup; `INSTALL.md` is the single install doc. |
