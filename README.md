...
2. Add the OAuth scopes in the app's Permissions page: under User identity API add `read:me`; under Jira API add `read:jira-work` and `read:jira-user`. Keep `offline_access` in `ATLASSIAN_SCOPES` so the app receives refresh tokens.
