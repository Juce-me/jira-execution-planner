...
| Cache-first Sprint startup restoration | `frontend/src/dashboard.jsx`, `backend/services/sprints.py` | Restoring the saved Sprint label and established cached catalog request changes passive startup behavior only. Existing `filter_changed` remains limited to user-committed Sprint choices; startup restoration emits no event and never sends Sprint ids or names. | 2026-09-10 |
