...
- [Persistent Sprint and per-Sprint Team catalogs](EXEC-persistent-sprint-team-catalogs.md): implementation-ready after the 2026-09-14 contract amendment against `d9da7bf` on `bugfix/board-progressive-loading`; implementation remains unexecuted. The four gaps now have explicit browser identity/empty/generation rules, one bounded refresh-completion protocol, the existing budget-aware DB OAuth read adapter, and a common PostgreSQL fence covering first configuration creation and all three current writers. Workspace sharing remains settled; the merged selector stays intact and its full five-mode/readiness matrix is required with DB catalog-state overlays. Real PostgreSQL races, new red/green/browser tests, authenticated read-only acceptance and timings remain execution checks. The separate cold ENG name-warm plan stays deferred. No Home gate sweep or mutation probe belongs to this amendment.
