...
| `settings_action` | `section`, `workflow_action`, `result`, `validation_count_bucket`, `dirty_state`; Department visibility and first-run selection use bucketed `selected_count_bucket`/`group_count_bucket` only | Settings tab open, test, save, cancel, validation failure, Department visibility preference change, first-run Department selection completion, or shared excluded-capacity epic toggle from Planning/Reporting | `frontend/src/settings/*`, `frontend/src/dashboard.jsx` | browser | Settings reliability |
...
| Lead Times capacity cohort filter | `frontend/src/dashboard.jsx` | The Lead Times Capacity selector (`all`/`ad_hoc`) only updates local cohort filter state to re-slice already-fetched issues; it triggers no scope re-fetch and no new user action, so it reuses the surrounding page/`api_result` analytics and sends no epic keys, summaries, or team names. | 2026-06-19 |
