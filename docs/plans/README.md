...
13. `EXEC-user-onboarding-tour-improvements.md`
   - Tasks 0–8 are implemented on `feature/user-onboarding-tour`. The dashboard tour is desktop-only; first-run Department selection and configuration still work on mobile, while the mobile dashboard tour is deferred in GitHub issue #151.
   ...
   - Task 9 remains pending: create the authorized fresh verification worktree at a concrete implementation ref, use Node 20 for dependency installation and reproducible builds, run the complete Python, frontend unit, and Playwright suites, generate and inspect every settled screenshot, confirm generated-output reproducibility, and complete the final diff review. Focused checks from Tasks 0–8 are implementation evidence only; they do not establish the pending full-suite, screenshot, or release accessibility gates. Current external Source Copy and flaky Board gates must be reported separately if they affect Task 9; they are not treated as passed here.
