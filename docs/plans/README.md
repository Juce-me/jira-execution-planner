...
- [Persistent Sprint and per-Sprint Team catalogs runtime repair](EXEC-persistent-sprint-team-catalogs-runtime-repair.md): planned investigation and repair for the strict catalog query-parameter mismatch that currently causes authenticated `/api/sprints` and `/api/teams` requests with legacy cache-busters to return `400`. Requires red/green wrapper coverage, rebuilt production bundle, local PostgreSQL/OAuth startup, and the outstanding two-user/cache-latency acceptance before the feature can be reported as working.
