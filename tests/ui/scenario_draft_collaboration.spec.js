...
test('global auth lock closes Scenario SSE and suppresses callbacks, polling, and keyboard effects', async ({ page }) => {
    await page.addInitScript(() => {
        window.SCENARIO_DRAFT_SSE_ENABLED = true;
        window.__scenarioStreams = [];
        window.EventSource = class FakeEventSource {
            constructor(url) {
                this.url = url;
                this.closed = false;
                this.listeners = new Map();
                window.__scenarioStreams.push(this);
            }
    ...
    await dragScenarioBar(page);
    await expect(page.locator('.scenario-dirty-indicator', { hasText: '1 override' })).toBeVisible();
    await expect.poll(() => page.evaluate(() => window.__scenarioStreams.length)).toBeGreaterThan(0);

    await page.route('**/api/analytics/context', route => json(route, { error: 'auth_required' }, 401));
    await page.evaluate(() => window.JepAnalytics.refreshAnalyticsContext().catch(() => {}));
    await expect(page.getByRole('alertdialog')).toBeVisible();
    await expect.poll(() => page.evaluate(() => window.__scenarioStreams.every(stream => stream.closed))).toBe(true);

    const pollsAfterLock = calls.events.length;
    await page.evaluate(() => {
        const stream = window.__scenarioStreams.at(-1);
        stream.emit('presence.updated', { data: JSON.stringify({
            eventNumber: 999,
            eventType: 'presence.updated',
            draftRevision: 5,
            payload: { presence: { displayName: 'Post Lock Editor', mode: 'editing' } },
        }) });
    });
    await expect(page.getByText('Post Lock Editor')).toHaveCount(0);
    await page.getByRole('alertdialog').dispatchEvent('keydown', { key: 'z', ctrlKey: true, bubbles: true, cancelable: true });
    await expect(page.locator('.scenario-dirty-indicator', { hasText: '1 override' })).toBeVisible();
    await page.waitForTimeout(5200);
    expect(calls.events.length).toBe(pollsAfterLock);
});
