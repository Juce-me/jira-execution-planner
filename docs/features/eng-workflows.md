...
The dashboard restores the saved Sprint label immediately, but it does not load or display ENG Jira work until a non-empty Sprint catalog validates that selection. OAuth requests reuse a user-partitioned in-process catalog for 24 hours; a cache miss resolves the actual quarterly Sprint values from Jira. While discovery is pending, or when Jira returns no usable Sprint values, the Sprint selector and all ENG work remain blocked instead of opening a perpetual loading menu.

The two cross-sprint choices become selectable only after strict Board capability is enabled and the Sprint catalog is ready. They also require Department Board columns plus Jira project authority from either saved selected projects or a saved Jira source Board. Component additionally requires configured Components; All work requires configured Components or Teams.
