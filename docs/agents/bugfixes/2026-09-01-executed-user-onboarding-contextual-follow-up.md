...
Partially superseded by the 2026-09-02 screen-scoped follow-up. Contextual modules, the single favorite control, accessibility restoration, analytics, and no-mutation contracts remain current. The locked-Next and launcher-sequence decisions above are historical: Next now stays enabled on every step; Next or the highlighted field opens a read-only field preview; a preview choice/surface, the same field, or Next then advances exactly once; Escape closes without advancing. Catch Up never asks the user to activate Settings or another tool, and its final informational steps return to real header controls without opening another surface. `docs/features/onboarding.md` is the canonical current behavior.
