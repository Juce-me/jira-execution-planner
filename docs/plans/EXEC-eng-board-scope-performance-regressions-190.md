...
> **Selector supersession (2026-09-14):** [ENG shared Sprint selector availability](DONE-eng-board-scope-selector-availability.md) supersedes this plan's requirement that missing authority, missing Department prerequisites, or unresolved capability disable Component/All work and make their handlers no-op. The implemented selector now accepts the user's scope intent, preserves the saved ordinary Sprint, and shows an explicit loading/setup/error state while issuing zero strict or ordinary fallback requests until authority is ready. This plan's backend authority, Sprint-catalog, stream, measurement, resource-limit, and historical verification evidence remain current; its old disabled/no-op UI assertions must not guide future work.
