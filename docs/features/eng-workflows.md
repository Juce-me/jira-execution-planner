...
The shared Sprint selector remains clickable while sprint discovery is pending or returns no options. It shows **Loading…** while closed, and its open menu shows **Loading sprints...** or **No sprints available**, so the control never becomes an inert loading indicator. Jira fallback discovery requests only issues with a Sprint value in one complete paginated scan. Repeated pagination tokens fail fast, and a browser request that has not completed within 30 seconds stops with **Retry sprints** instead of remaining permanently in the loading state.
