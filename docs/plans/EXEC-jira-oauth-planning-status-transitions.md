...
- [x] **Step 7.1: Run focused backend tests**
...
- [x] **Step 7.2: Run focused frontend tests**
...
- [x] **Step 7.4: Run visual/interaction tests**
...
- [x] **Step 7.5: Ratchet structure budgets if entrypoints grew**
...
- [x] **Step 7.6: Run full test suite before push**
...
- [x] **Step 7.7: Server startup gate**
...
- [x] Catch Up mode can change status for one Epic through signed-in-user Jira OAuth.
- [x] Catch Up mode can change status for one Story through signed-in-user Jira OAuth.
- [x] Catch Up mode can change status for one expanded Subtask through signed-in-user Jira OAuth.
- [x] Planning mode can change status for one selected Story through signed-in-user Jira OAuth.
- [x] Planning mode can include an Epic in status targets and transition it without affecting selected story points.
- [x] Planning mode can include an expanded Subtask in status targets and transition it without affecting selected story points.
- [x] Batch mode sends every selected Story, Epic, and Subtask target in one request and never silently truncates the selection.
- [x] EPM remains view-only while `GATE-05` is blocked: EPM status pills/text are inert, no EPM transition API calls occur, and no EPM mutation routes exist.
- [x] Status pills/text are inert in Stats, Scenario, Settings, and unknown/non-ENG surfaces.
- [x] The server rejects over-cap batches before any mutation.
- [x] The write route requires OAuth mode, `X-Requested-With`, token-bound CSRF, and the signed-in user's Jira OAuth token.
- [x] The implementation never uses Jira Basic credentials, service integrations, Home/Townsquare credentials, or local token-store helpers for the mutation.
- [x] Transition IDs are resolved by the backend at write time from Jira transitions available to the user.
- [x] An issue already in the requested status is reported as `already_in_status` success, not failure.
- [x] Per-issue Jira transition GET/POST calls are bounded by a worker cap and carry request-scoped auth context, proven by a no-request-context test.
- [x] The write route returns `401 missing_oauth_scope` before any Jira POST when the session lacks `write:jira-work`.
- [x] Structure budgets pass: `frontend/src/dashboard.jsx` budget is ratcheted with a justifying comment and `jira_server.py` did not grow.
- [x] Per-issue failures are visible and retryable without hiding successful transitions.
- [x] Jira-derived caches are invalidated after successful transitions.
- [x] Analytics sends no issue keys, summaries, transition IDs, JQL, Jira URLs, assignees, emails, account IDs, tokens, or raw Jira error text.
- [x] `npm run build` regenerates `frontend/dist/` and leaves no post-build diff beyond committed generated output.
- [x] Catch Up, Planning, and EPM layouts are visually verified; Planning sticky layout is verified in collapsed and expanded Epic states.

## Outcome

Implemented as planned (with two documented refinements). Executed on branch
`docs/jira-oauth-planning-status-transitions` (base `dbc4463`) across Tasks 1-7,
each with an implementer + task review (spec + quality) loop. The shipped code,
tests, and generated `frontend/dist` are now the source of truth.

Refinements vs. the original plan text:
- The `clear_jira_issue_status_caches` cache helper lives in
  `backend/routes/eng_routes.py` (not `jira_server.py`) so the zero-headroom
  `jira_server.py` line budget did not grow; `jira_server.py` was not modified.
- Two new files beyond the original File Map were added as clean decompositions:
  `frontend/src/issues/StatusTransitionMenu.jsx` (the shared popover) and the CSS
  partials `frontend/src/styles/eng/status-transitions.css` +
  `frontend/src/styles/planning/actions.css` (imported by `eng.css`/`planning.css`).
  `INSTALL.md` was also updated for scope-string consistency (operator doc).

Verification at completion: focused backend 114 OK; focused frontend 141 pass;
full Python suite 997 OK (1 pre-existing skip); full Node suite 474/474; required
Playwright specs 23 passed (`eng_status_transitions` 15 + `planning_selection_defaults` 8);
structure budgets pass (`dashboard.jsx` ratcheted 15729->15866 with a justifying
comment, `jira_server.py` unchanged); `npm run build` regenerated committed dist;
startup preflight PASS and the server boots clean with `/api/test` returning the
correct `401 auth_required` for a no-session OAuth request.
