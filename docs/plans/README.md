...
- [ENG shared Sprint selector availability](EXEC-eng-board-scope-selector-availability.md): planned, not implemented. Live Board no-op behavior reproduced with a valid Sprint catalog; separates selectable scope intent from load authority, adds explicit configuration states and keyboard semantics, and preserves ordinary Sprint access across all five ENG modes. Supersedes only the prior no-op selection requirement; Sprint catalog implementation remains separately owned.
