...
> **Status: Implemented and verified on `improvement/eng-board-scope-selector-availability`; awaiting merge and acceptance.** The full implementation branch starts from baseline `6aaf9dac117d4708845f0202367e77e00ec24247`; Task 5 started from `fed8b369b2fdad1d9c873a8dca8b8d2c8f63ab3d`. Verification completed 2026-09-14. The selector, readiness model, scheduling protections, acceptance matrix, analytics contract, and documentation are complete on the branch. Keep this file as `EXEC-*` until the result is merged and accepted. Positive authenticated cross-sprint loading remains unverified because no existing authorized live scope was available without changing shared settings.
>
> **Execution:** Tasks 1–5 are recorded as five branch commits: `a342077` (`test: define blocked Board selector contract`), `83083a7` (`Separate ENG Board scope readiness`), `9cff873` (`Implement semantic Board scope selector`), `fed8b36` (`Fix Board scope scheduling`), and the containing Task 5 `HEAD` commit (`test: complete Board selector acceptance matrix`). Execution used no worktree, shared configuration change, Jira write, backend contract change, push, or publication. This plan supersedes only the disabled/no-op scope-selection requirements in [the scope follow-up](EXEC-eng-board-scope-performance-regressions-190.md); its backend authority and measurement protections remain required. [Progressive loading](EXEC-eng-board-progressive-loading-190.md) remains authoritative for the stream, bounds, incomplete results, and measurement.
...
This is the complete 29-path implementation diff from `6aaf9dac117d4708845f0202367e77e00ec24247` through branch `HEAD`. `Create` entries are deliberate new files. No catalog or backend production file changed.
...
| Modify | `docs/plans/EXEC-eng-board-scope-selector-availability.md`, `docs/plans/README.md`, `docs/plans/EXEC-eng-board-scope-performance-regressions-190.md` | Record actual execution/outcome and supersession of only the obsolete no-op requirement |
...
- `env CONFIG_STORAGE_BACKEND=jsonfile .venv/bin/python -m unittest tests.test_eng_board_routes tests.test_eng_board_service tests.test_eng_board_source_guards tests.test_eng_board_basic_compat` — 107 passed.
...
- `npx --no-install playwright test tests/ui/eng_group_board_view.spec.js tests/ui/eng_board_stream.spec.js tests/ui/eng_board_progressive_loading.spec.js --workers=1` — 161 passed serially in 3.5 minutes. Chromium required the approved local process/IPC run. The runner warned that `NO_COLOR` was ignored because `FORCE_COLOR` was set.
...
- `env CONFIG_STORAGE_BACKEND=jsonfile .venv/bin/python -m unittest discover -s tests` — 1,797 tests, 9 skipped. The selector/readiness growth and evidence-driven Project Track corrections are documented in `tests/test_codebase_structure_budgets.py`; final full discovery passed at the explicit 18,025-line budget. Existing negative-path logs and static-file `ResourceWarning`s remain warnings only.
- `git diff --check` passed, and the final committed-revision `git diff --name-status 6aaf9da..HEAD` audit accounted for all 29 changed paths in the file map.
