...
| --- | --- | --- | --- |
| ENG Board Sprint readiness, cross-sprint availability, progressive discovery, loading animation, and retained partial results | `frontend/src/dashboard.jsx`, `backend/routes/settings_routes.py`, `backend/routes/eng_board_routes.py`, `backend/services/eng_board.py`, `frontend/src/eng/useEngBoardData.js`, `frontend/src/eng/loadPerformance.js`, `frontend/src/eng/EngBoardView.jsx` | Sprint cache hits/misses, pending/empty catalog gates, capability, saved Jira project/Board authority, Department Board columns, candidate indexes, Component pages, Team-parent batches, provisional counts, loading bars, paint observations, and incomplete/stale presentation are passive state and emit no new event. A disabled selector or disabled All work/Component click emits no `filter_changed`; an accepted explicit choice retains the existing `trigger=userevent`, `event_type=event`, canonical `event_name=filter_changed`, `filter_type=sprint`, `source_surface=board`, and bounded `scope_type`. The terminal `api_result` contract is unchanged: `feature_name=eng_board`, `api_surface=eng_board`, bounded `scope_type`, existing method/status/result/duration/cache parameters, and one terminal numeric duration, at most once for an accepted current generation and never for superseded or auth-locked loads. Operational observations remain outside GA4. This corrective cache/readiness/availability behavior therefore requires no taxonomy, custom-definition, transport, or runbook change. Never send Jira identifiers, frame payloads, names, JQL, URLs, or raw errors. | 2026-09-12 |
