...
> **Current accuracy:** This file remains authoritative for per-user Home/Townsquare credentials, read authorization, and private EPM saved-view ownership. PR #130's temporary shared-EPM boundary is superseded by `EXEC-user-owned-epm-configuration.md`; EPM scope, label mask, issue-type mapping, project-label mapping, tab, and sprint remain private to the owning user.
