...
> **Status:** Implemented and locally verified on `bugfix/board-progressive-loading` on 2026-09-22. Workspace-shared ownership is confirmed and unchanged. Keep this plan as `EXEC-*` until the remaining authenticated Jira, two-user/multi-workspace, and cached-latency acceptance gates are completed and the change is accepted or merged.
...
**Status: implemented locally; external acceptance remains open.** The four 2026-09-13 P1 findings are closed by the contracts and implementation below. Real PostgreSQL races and the new automated suites are verified; authenticated read-only Jira/browser acceptance and latency measurements remain mandatory release acceptance.
...
## Independent Revalidation — 2026-09-20

**Status: implementation-ready after the plan-only corrections below; implementation and acceptance remain unperformed.** The branch was fast-forwarded from `d9da7bf` to origin commit `a3ec68f`. Three independent read-only reviews traced the current auth/deadline path, browser authority/imports and PostgreSQL writers/migrations before this plan was amended. The eight reported findings now resolve as follows:

| Reported finding | Revalidated disposition |
| --- | --- |
| Amendment prerequisite/status | Resolved: this plan and the index state implementation-ready but unimplemented; every execution task remains unchecked. |
| Deadline contract | Resolved: Tasks 3–5 use existing budget-aware `current_jira_get`/`current_jira_search` with explicit context/transport and prohibit `current_jira_request`; real-wrapper tests cover DB locks, OAuth, retries, bodies and publication. |
| Async Sprint completion | Resolved: one attempt/identity-based, finite, cancellable observer has explicit terminal/exhausted states and no refresh admission from comparison reads. |
| Browser Sprint authority | Resolved: server identity/browser context, display-only restoration, accepted-source generations and Board-save invalidation prevent stale Board authority. |
| Validated empty readiness | Resolved: an empty current snapshot is validated and terminal, clears selection and does not refill; work readiness still requires a selected member. |
| First configuration insert | Resolved: the common fence covers all three current first-writer paths and catalog claim/publication with an exact lock order and real PostgreSQL barriers. |
| Frontend file map | Resolved: Sprint work owns `engApi.js`; Team work owns `jiraCatalogApi.js`; runtime/dashboard/tests are explicit. `configApi.js` is now a read/verify dependency for bootstrap source pass-through. |
| Error contract | Resolved: full sanitized cache/attempt metadata is present whenever identity is known; source/removal/storage/auth conflicts remain fixed-shape and non-leaking. |

The revalidation found and closes seven narrower gaps not explicit in the original list: a live PostgreSQL Alembic lifecycle proof, exact persisted-state CHECK truth tables, promotion fence-timeout rollback, fresh-page authority for a failed envelope that still contains a valid snapshot, forced Retry semantics, browser-session logout/publication serialization, and explicit config-bootstrap source pass-through coverage. These are binding changes below, not implementation evidence. As of this 2026-09-20 revalidation, `GATE-05` remained blocked and was next due 2026-09-21; the 2026-09-21 startup review kept it blocked and set its next review to 2026-09-28. This read-only Jira catalog plan neither runs nor depends on the Home write probe.
...
Both store independent attempt bookkeeping: UUID `refresh_attempt_id`, `attempt_identity`, `attempt_config_digest`, UTC `attempt_deadline_at`, `refresh_status` (`idle|pending|completed|failed`), nullable allowlisted `failure_code` (`refresh_budget_exhausted|catalog_refresh_lock_timeout|oauth_refresh_timeout|jira_unavailable|catalog_incomplete|catalog_identity_changed|auth_required|catalog_runtime_unavailable`), nullable `last_failure_at`, `retry_at`, UUID `refresh_lease_owner`, UTC `refresh_lease_until`. Team attempts store exact column `attempt_scope_digest` separately from validated `scope_digest`; Sprint rows omit that column. `retry_at` is the sole failed-attempt retry time (no duplicate `next_attempt_at`). Payload length is never a validation flag.

Database CHECK constraints implement this exact truth table, and the live PostgreSQL migration test below must prove both valid fixtures and rejection of every invalid cell:

| Stored state | Required metadata | Forbidden metadata |
| --- | --- | --- |
| Any row | Positive-decimal `board_id` or `sprint_id`; `refresh_status` and nonnull `failure_code` values must be allowlisted; `validated_at` and `catalog_version` are both null or both nonnull. A validated Team payload also has nonnull `scope_digest`. | Non-decimal/zero/negative IDs; half-populated validation pairs; unknown status/failure codes. |
| `idle` | No attempt metadata. | Attempt ID/identity/config digest/deadline, Team attempt scope, lease owner/deadline, or failure/retry metadata. |
| `pending` | Attempt ID, identity, config digest and deadline; lease owner and lease deadline; Team attempt scope digest. | Failure code/time/retry metadata. |
| `completed` | Retained attempt ID, identity, config digest and deadline; Team attempt scope digest. | Lease or failure/retry metadata. |
| `failed` | Retained attempt ID, identity, config digest and deadline; allowlisted failure code, `last_failure_at` and `retry_at`; Team attempt scope digest. | Lease owner/deadline. |

Owner/deadline, attempt identity/deadline, validation/version and all status-specific groups are atomic pairs/groups under those constraints. Model validation mirrors the database but never substitutes for PostgreSQL constraint proof.
...
Lock order: workspace advisory fence → fresh refresh-actor rows (when applicable, `User` SHARE then `AuthConnection` SHARE then captured `BrowserSession` SHARE) → effective configuration row if present → Sprint row → Sprint-Team row → directory row. Acquire only rows needed by the operation, in that order. The advisory fence exists even when every derived/config row is absent. `BrowserSession` has no independent expiry column: when `context.browser_session_id` is present, publication requires a fresh `SELECT ... FOR SHARE` proving exact ID/user/workspace/connection binding. The row lock, not the workspace fence, linearizes publication with the existing logout deletion. Auth token refresh completes its separate transaction before acquiring this fence; never nest the token-refresh lock inside it.
...
| Current HTTP 200 failed envelope with matching validation | Accept its valid payload as the current document's `validatedSnapshot` and set `authority=validated,status=error`; nonempty selected membership remains usable while fixed non-blocking failure copy is shown. This includes the first server read after display-only restoration. |
...
| Transport failure, payloadless 502/503, malformed envelope or client observer exhaustion | Keep only a previously validated matching context/identity snapshot; status error/exhausted; never create authority from display data or missing validation; show fixed retry copy |
...
- On exhaustion set `status=exhausted`, detach observer and show **“Sprint refresh is taking longer than expected. Retry.”** Failure copy is **“Sprint refresh failed. Retry.”** No valid snapshot: **“Sprint catalog is unavailable. Retry.”** Board removed: **“Choose a Jira source Board in Settings.”** Never show upstream errors/IDs. Every Sprint catalog Retry control calls `controller.refresh()` and sends exactly one `refresh=true` initiating request, never a normal or completion read. If a live attempt exists the server response observes/coalesces onto it and cannot create a competing lease; after terminal failure/exhaustion the new generation may bypass `retry_at`, subject to admission and lease guards. No Retry starts an automatic loop.
...
`assert_catalog_refresh_actor(session, context, *, budget=None, lock_for_publication=False)` in `workspace_catalog_cache.py` directly checks fresh active User, OAuth AuthConnection/provider, workspace/site/cloud/user binding, provider-verified required scopes and, when present, fresh BrowserSession existence plus exact ID/user/workspace/connection binding. BrowserSession has no separate expiry field. Invoke it on every catalog read (including cached/completion reads, without token resolution or Jira), before every logical upstream call and publication; publication passes `lock_for_publication=True` and takes the shared actor-row locks in the common order above. Do not use `db_context`'s status cache or token-version-partitioned Board `_assert_current`. Workers always pass their original budget; DB-only readers may omit it. Legitimate rotation may change `token_version`; it must not make its own worker stale. Revocation, disabled user or deleted captured browser session blocks reads, new work and publication, but never deletes saved shared rows. Token material stays in DB `auth_connections`/encrypted `auth_tokens`.
...
| Modify | `frontend/src/dashboard.jsx` | `loadConfig:6716`, `loadSprints:7044`, `renderSprintControl:14050`, `refreshActiveViewFromJira:14832`; selected intent/readiness preserved; Team paths `loadTeamsFromCurrentView:2625`, `fetchAllTeamsFromJira:2633`, `resolveMissingTeamNames:2700`, save branch `:3087` |
...
| Create | `tests/test_workspace_catalog_migration_postgresql.py` | Required isolated-schema Alembic upgrade/downgrade/re-upgrade and live CHECK/FK/index/default proof |
...
| Modify/test | `tests/test_oauth_stats_routes.py` | Current non-DB OAuth Sprint route/cache fixtures; keep separate from new DB cases |
| Modify/test | `tests/test_oauth_eng_routes.py` | Current non-DB OAuth Team, resolve, all-Team and `all=true` fallback compatibility fixtures |
...
Read/verify only (no interface change needed): `backend/config/db_repository.py` (`DbConfigRepository.load_dashboard_config_snapshot:67`, `save_dashboard_section:82`, `save_team_catalog:96` delegate to preserved wrappers); `backend/config/import_config.py` (private import strips shared sections); `backend/auth/context.py`, `backend/auth/db_context.py`, `backend/auth/db_tokens.py`, `backend/auth/jira_auth.py`, `backend/auth/home_credentials.py`, `backend/auth/db_browser_sessions.py`, `backend/db/engine.py`, `backend/jira_client.py`, `backend/services/eng_board_stream.py`, `frontend/src/api/configApi.js` (`normalizeAppConfig` must pass through exact `sprintCatalogSource`), `frontend/src/api/http.js`, `frontend/src/api/authRequired.js`, `frontend/src/eng/engSprintSelectorState.js`, `frontend/src/eng/useEngSprintData.js`, `frontend/src/eng/useStrictEngBoardIntegration.js`, `frontend/src/eng/useEngBoardData.js`.
...
| `loadTeamsFromCurrentView` | Task-derived labels are names-only display input; never grants Sprint membership or availability |
...
**Create:** `backend/db/migrations/versions/20260913_0016_workspace_sprint_team_catalogs.py`, `tests/test_workspace_catalog_migration_postgresql.py`. **Modify:** `backend/db/models.py`, `runners/github/run-postgresql-tests.sh`, `tests/test_postgresql_runner_contract.py`. **Tests:** `tests/test_db_migrations.py` (`DbMigrationTests`) plus the required live PostgreSQL module. Consumes existing workspace/user FK conventions; produces the two Storage Design models/constraints, no imported data.

- [ ] RED: write `test_catalog_migration_empty_upgrade_downgrade_and_reupgrade`, `test_catalog_model_identity_attempt_and_lease_constraints`, `test_catalog_migration_postgresql_offline_sql`. Assert exact columns/defaults/indexes/FK actions/unique keys, every Storage Design truth-table cell, version-1 JSON `[]`, no data movement and no user-scoped uniqueness. Run `.venv/bin/python -m unittest tests.test_db_migrations`; new tests must fail for absent revision/tables, while existing migration cases retain their assertions. SQLite/model tests are functional feedback only, never proof of PostgreSQL CHECK behavior.
- [ ] GREEN: implement models and migration with `down_revision='20260908_0015'`. Downgrade drops only the two derived tables; it never modifies names, configuration or credentials. Re-run the same command; expect all passing.
- [ ] Add live `WorkspaceCatalogMigrationPostgresqlTests.test_real_alembic_catalog_upgrade_downgrade_reupgrade` using the existing guarded disposable `TEST_DATABASE_URL` and an isolated random schema. Run real Alembic to `20260913_0016`; inspect exact FKs/actions, unique keys, indexes, server defaults and CHECK constraints; insert valid rows and assert the database rejects every invalid truth-table cell. Seed synthetic existing workspace configuration and Team directory rows, downgrade to `20260908_0015`, prove only the two derived tables disappear and seeded existing rows survive, then re-upgrade and prove both derived tables are recreated empty. Never use `models.Base.metadata.create_all` as migration evidence.
- [ ] Wire `tests.test_workspace_catalog_migration_postgresql` into `runners/github/run-postgresql-tests.sh` and its exact runner-contract test under `REQUIRE_POSTGRES_CATALOG_CONCURRENCY=1`. Missing/non-PostgreSQL targets fail, not skip. The explicit Task 9 command is mandatory release evidence.
...
assert_catalog_refresh_actor(session, context, *, budget=None,
                             lock_for_publication=False) -> None
```
...
| `test_promotion_fence_timeout_exits_nonzero_without_insert` | Hold the workspace fence on another connection, run fingerprint-authorized promotion apply with its bounded timeout, and assert nonzero exit plus no configuration row or audit event. Release and clean only the owned fixture. |
| `test_browser_logout_serializes_with_catalog_publication` | Prove both orderings with actual browser-session deletion. Publication holds BrowserSession SHARE and logout is observed waiting, then publication commits before logout. When logout commits first, fresh publication actor validation finds no row, rejects publication and leaves payload/version/directory unchanged. Do not add the workspace fence to logout. |
...
- [ ] RED endpoint cases in `PersistentCatalogRouteTests`: `test_stale_hit_returns_before_held_worker`, `test_completion_pending_completed_and_failed_keep_same_attempt`, `test_completion_reads_never_admit_jira`, `test_completion_rejects_wrong_identity_new_attempt_and_mixed_force`, `test_cold_fill_empty_and_failed_backoff`, `test_force_retry_bypasses_failed_backoff_but_never_active_lease`, `test_cached_user_b_reuses_user_a_snapshot`, `test_revoked_reader_cannot_read_shared_hit`, `test_removed_board_409_never_returns_old_list`, `test_cache_hit_auth_and_field_resolution_do_not_call_jira`. Assert every Cache field/status/header, current workspace comparison, no raw error payload and one owner across manual/ordinary reads. The force-retry case seeds a future `retry_at`: force may claim one new attempt only without a live lease; with a live lease it returns/coalesces onto that attempt. Seed actual DB OAuth connections/tokens; the existing OAuth stats module clears DB config and is compatibility evidence only. Run `.venv/bin/python -m unittest tests.test_persistent_catalog_routes`; expected RED missing catalog runtime/envelope assertions, not live Jira.
...
- [ ] RED pure cases (literal Node test names): `restored Sprint catalog remains display-only until server validation`, `populated Board A to validated empty Board B clears authority`, `failed server envelope with validated payload establishes authority after display-only restore`, `failed payloadless envelope cannot establish authority`, `removed Board invalidates restored and validated choices`, `delayed Board A success and failure cannot change Board B`, `same-identity transient failure retains validated snapshot`, `auth context change invalidates authority without changing shared ownership`, `new generation suppresses stale finally and persistence`, `completion exhaustion is terminal`, `Retry after failure or exhaustion sends one forced read and coalesces a live attempt`, `manual refresh never treats ordinary read as completion`. Use injected clock/timers/deferred reads; assert state and request counts, not timer implementation details. Current restore test's 24-hour display TTL remains, but its nonempty authority assumption is removed.
- [ ] RED wrapper cases: `Sprint completion reads pass signal and attempt identity without refresh`, `Sprint forced refresh rejects completion parameters`, `Sprint completion uses apiFetch global auth protection`, `config bootstrap preserves exact Sprint catalog source`. Assert encoded query and same AbortSignal. The bootstrap source case calls current `normalizeAppConfig` through `fetchAppConfig` and proves exact `sprintCatalogSource` pass-through without assigning new ownership to `configApi.js`. Run `node --test tests/test_dashboard_runtime.js tests/test_frontend_api_source_guards.js tests/test_eng_sprint_selector_state.js`; unsafe authority/missing coordinator fail; existing selector pure behavior remains green.
...
| `catalog refresh: initial failed validated snapshot remains usable` | Start with display-only storage, return current HTTP 200 `state:failed` plus valid version/list; all five modes may start their real work for the selected member while fixed non-blocking warning remains. Payloadless failed response starts none. |
...
| `catalog refresh: Retry forces one new attempt` | Seed a cold failed envelope with future `retryAt`, click visible Retry, assert exactly one `refresh=true` initiating request and a new attempt when no lease is live; a live attempt is observed/coalesced, and no normal/completion request masquerades as Retry. |
...
**Create:** `frontend/src/settings/teamAvailability.js`, `tests/test_team_availability.js`. **Modify:** `frontend/src/api/jiraCatalogApi.js`, `frontend/src/dashboard.jsx`, `tests/test_frontend_api_source_guards.js`; **browser tests:** `tests/ui/settings_unified_save.spec.js`, `tests/ui/shared_department_groups.spec.js`. Current `loadTeamsFromCurrentView:2625`, `fetchAllTeamsFromJira:2633` and `resolveMissingTeamNames:2700` must no longer conflate names with availability. Task-derived Teams remain names-only display input.
...
**Modify:** `frontend/src/settings/TeamGroupsSettings.jsx`, `frontend/src/settings/FirstRunGroupConfigurationGuide.jsx`, `frontend/src/settings/FirstRunGroupSelectionModal.jsx`, `frontend/src/settings/useGroupVisibilityPreferences.js`, `frontend/src/styles/settings/team-selector.css`, `frontend/src/dashboard.jsx`; **tests:** `tests/test_first_run_group_configuration.js`, `tests/ui/settings_unified_save.spec.js`, `tests/ui/shared_department_groups.spec.js`. Consumes Task 6 availability only; no endpoint/permission/dirty-state redesign.
...
REQUIRE_POSTGRES_CATALOG_CONCURRENCY=1 .venv/bin/python -m unittest -v tests.test_workspace_catalog_migration_postgresql
REQUIRE_POSTGRES_CATALOG_CONCURRENCY=1 .venv/bin/python -m unittest -v tests.test_workspace_catalog_postgresql
```

The third and fourth commands require an already-provisioned disposable test DB; do not infer an application database target. The migration command must execute the real isolated-schema Alembic lifecycle, and every named migration/race test must run with zero skips. CI runner includes both modules and the required flag. Document the database engine and actual result without credentials.
...
### Local implementation record — 2026-09-22

Tasks 1–8 are implemented on the named branch. The plan remains `EXEC-*` because the external acceptance items in Task 9 are not available from this unauthenticated local session.

- Focused backend catalog/config/service/route checks passed **306 tests**; auth and Board checks passed **106 tests** with **3 skipped**.
- The full Python suite passed **1,907 tests** with **25 skipped** when optional external database environment variables were cleared. Existing static-request diagnostics emitted `ResourceWarning` messages; no test failed.
- Live PostgreSQL 18 migration and concurrency verification passed **16 tests with zero skips**, including real Alembic upgrade/downgrade/re-upgrade, first-writer fences, actor/logout serialization, budget contention, supersession, and atomic publication.
- Frontend unit verification passed **1,423/1,423** tests. The production build completed, and structure budgets passed at `jira_server.py` **6,449/6,461** lines and `frontend/src/dashboard.jsx` **18,041/18,078** lines.
- Every mapped browser case passed on the final production bundle: the full Settings spec passed **50/50**, the full shared Department groups spec passed **69/69**, and the other four mapped specs passed their **177** cases. The exact six-file single-worker aggregate is host-stall-prone in this session: its best run passed **295/296**, while a later run passed **294/296** and recorded multi-minute Chromium navigation/interaction stalls; each failed case passed in its complete independent spec or repeated focused run. Do not treat that aggregate harness instability as a green monolithic run.
- Startup preflight passed against the disposable PostgreSQL schema after Alembic upgrade, and Flask reached its startup banner without pre-banner runtime warnings. The unauthenticated `/api/test` check returned the expected sanitized `401 auth_required`; positive authenticated API acceptance remains open.
- Still required before `DONE-*`: authorized read-only two-user sharing and cross-workspace isolation against Jira, five cached request samples for both catalog endpoints with `Server-Timing`, zero ordinary-hit Jira calls, and positive authenticated `/api/test` evidence. No real Jira fixture or credential was stored.
...
| Browser state machine | Restore/empty/failed-with-validation/payloadless errors/identity/auth/generation/cleanup/forced Retry/superseded manual refresh and open-menu completion specified; existing selected intent preserved |
| Dirty/concurrent configuration | Common fence for all three writers, missing-row fallback and first publication; BrowserSession logout linearization; unchanged dirty/revision/conflict recovery; PG barriers prove no first-save interposition |
| Verification | Named RED/GREEN tests, actual module names, real-wrapper OAuth path, live Alembic lifecycle and CHECK truth table, strict PG no-skip requirement, five-mode/readiness overlays, screenshot and latency proof |
...
**Independent revalidation checks (2026-09-20, current implementation only):** the same four Python modules again passed **49 tests**, zero skipped, and `node --test tests/test_dashboard_runtime.js` passed **1 test**. The current Node 22.22.0 frontend runtime/selector/source-guard suite passed **66 tests** with the existing `MODULE_TYPELESS_PACKAGE_JSON` warning; relevant writer/migration/adapter characterizations also passed without application-file changes. `git diff --check`, placeholder scan, current migration-head check (`20260908_0015`) and exact planned Create-path presence/absence checks passed after the plan corrections. No future PostgreSQL migration/concurrency, browser implementation, server startup, authenticated Jira acceptance or latency result is inferred from these existing-code checks.

Pinned Node **20.20.2** was used for the accepted 2026-09-14 runtime-test rerun and `npm run build`; both passed. The initial host Node 26.8.1 run also passed, but does not replace pinned-runtime verification. All existing file-map paths and relative links in the three amended documents resolve; all **14** planned Create paths are absent as expected; all four JSON blocks parse. The build changed no generated files. No new implementation tests, PostgreSQL migration/concurrency, server-startup, browser campaign or latency acceptance is claimed by this task.
