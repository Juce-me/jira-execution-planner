...
The runner always binds Flask and PostgreSQL to `127.0.0.1` and accepts no overrides. When no runner process owns a complete, label-owned `jira-planning-local` Compose stack, startup removes that stale container and network before continuing while retaining the database volume. Partial, mismatched, or otherwise ambiguous existing resources still fail closed. The `jep`/`jep` database credentials are for this dedicated local container only. Docker access is host-administrator access, and the retained volume can contain sensitive local configuration or encrypted token records.
...
If startup reports ambiguous runner resources or says resources remain after targeted cleanup, inspect only the exact `jira-planning-local` Compose project named in the error. Do not use broad Docker prune commands or `backend.db.reset_local` as lifecycle cleanup.
